...
Public Class rubros
    ...
    Sub btnAgregar_Click()
        Dim insertar As New crubros
        Dim resultado As String = ""

        resultado = insertar.InsertarRubro(txtRubro.Text)
    ...
    End Sub
    Sub CargarGrilla()
        Dim cargar As New crubros
        Dim grilla As New DataSet
        grilla = cargar.cgRubro()
    ...
    Sub btnModificar_Click()
        ...
        resultado = modificar.ActualizarRubros(id, txtRubro.Text)

        If resultado = "ok" Then
            lblResultado.Text = "Se Modifico un Rubro, Código del Rubro=" & id & ""
            lblResultado.Visible = True
            txtRubro.Text = ""
    ...
    '        info = fila.Cells(0).Text
    '        ID = CInt(info)
    '        resultado = eliminar.eliminarRubro(ID)
    '    End If
    ...
    Protected Sub gvRubros_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gvRubros.SelectedIndexChanged
        ...
        leer = modificar.ComandosLectura(id)
            If leer.Read Then
                txtRubro.Text = leer.Item("rubroNombre").ToString
